...
def fetch_peers(peer):
    peer_list = requests.get('http://{}:{}/peers'.format(peer.host, peer.port),
...
def get_edges_from_peer(peer):
    return requests.get('http://{}:{}/edges'.format(peer.host, peer.port),
                        proxies=proxies).json()


def get_ports(peer):
    return requests.get('http://{}:{}/ports'.format(peer.host, peer.port),
                        proxies=proxies).json()


def get_containers(peer):
    return requests.get('http://{}:{}/containers'.format(peer.host, peer.port),
                        proxies=proxies).json()
